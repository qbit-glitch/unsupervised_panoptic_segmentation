...
\begin{abstract}
We present a fully unsupervised panoptic segmentation pipeline that composes frozen foundation models into a pseudo-label generation framework, then trains a panoptic network that surpasses the current state of the art without any manual annotation. Unsupervised panoptic segmentation remains challenging because it requires jointly recovering both semantic categories and individual object instances from unlabeled images---a problem that existing methods address through stereo video and optical flow, limiting their applicability to monocular settings. Our approach decomposes pseudo-label generation into two complementary streams: semantic labels are obtained by overclustering CAUSE-TR features with $k{=}80$ K-means centroids, recovering seven semantic classes that collapse under standard cluster counts; instance labels are derived from monocular depth maps via Sobel gradient thresholding and connected component analysis, exploiting depth discontinuities as object boundary proxies. The composed panoptic pseudo-labels are used to train a Cascade Mask R-CNN with a frozen DINOv3 ViT-B/16 backbone, followed by EMA-guided self-training over 8,000 steps. On the Cityscapes validation set under the standard 27-class CAUSE+Hungarian evaluation protocol, our method achieves a panoptic quality of \textbf{32.76\%}, surpassing CUPS (CVPR 2025) by \textbf{+5.0 PQ}---notably using only monocular images where CUPS requires stereo pairs and optical flow for pseudo-label generation.
\end{abstract}
...
We evaluate primarily on the Cityscapes~\cite{cordts2016cityscapes} validation set (500 images, 1024$\times$2048, 19 classes: 8~thing + 11~stuff). All results use the 27-class CAUSE+Hungarian evaluation protocol from CUPS~\cite{hahn2025cups}. Panoptic quality~\cite{kirillov2019panoptic} is defined as:
\begin{equation}
\text{PQ} = \frac{\sum_{(p,g)\in\text{TP}} \text{IoU}(p,g)}{|\text{TP}| + \tfrac{1}{2}|\text{FP}| + \tfrac{1}{2}|\text{FN}|} = \text{SQ} \times \text{RQ}
\label{eq:pq}
\end{equation}
We additionally evaluate on MOTS, KITTI, Mapillary Vistas~v2~\cite{neuhold2017mapillary}, and COCO-Stuff-27 for cross-dataset generalization. The DINOv3 backbone uses the publicly available \texttt{facebook/dinov3-vitb16-pretrain-lvd1689m} checkpoint. Network training runs on two GTX~1080~Ti GPUs (11\,GB each) with PyTorch~2.1.2 and Detectron2~0.6. All seeds are fixed at 42. At inference, the model runs at $\sim$2--3 FPS on a single GTX~1080~Ti at 640$\times$1280 resolution.

\paragraph{Comparison with state of the art.}

Table~\ref{tab:sota} compares our method against all published unsupervised panoptic segmentation methods on Cityscapes. Prior semantic-only methods (PiCIE, STEGO, HP) have no published PQ numbers.
...
Our Stage-2 result matches CUPS overall PQ (27.87 vs 27.8) while achieving substantially higher thing quality: \pqt=23.2 versus 17.7 (+5.5 points). Stage-3 self-training raises PQ to 32.76, a gain of +4.9 over Stage-2 and +5.0 over CUPS. The stuff quality (32.0) falls below CUPS (35.1) by $-$3.1 \pqs; a per-class analysis reveals this gap is entirely attributable to 7 non-standard CAUSE classes on which our model achieves PQ below 1\%, whereas CUPS's end-to-end trained semantic head learns them directly. On the 16 classes with PQ above 10\%, our average PQ is 49.0\%.

We note that while our method requires only monocular input, DAv3 was itself pretrained with multi-view geometry---the ``monocular'' characterization refers to deployment requirements, not to foundation model pretraining.

\begin{table*}[t]
\centering
\caption{Ablation studies on Cityscapes val. (a)~Pseudo-label quality before network training. (b)~Depth model effect on instance \pqt at optimal $\tau$. (c)~Training backbone ablation (Stage-2 only). (d)~Self-training scaling across backbones.}
\label{tab:ablations}
\small
...
ViT-B/16 & \textbf{27.87} & \textbf{23.2} & \textbf{30.6} \\
\bottomrule
\end{tabular}
\end{minipage}
&
...
\begin{minipage}[t]{0.46\textwidth}
\centering
\textbf{(b) Depth Model Effect} \\[4pt]
\begin{tabular}{@{}lcc@{}}
\toprule
Depth Model & $\tau$ & \pqt \\
\midrule
None (CC only) & --- & 14.93 \\
SPIdepth~\cite{lavreniuk2025spidepth} & 0.20 & 19.41 \\
DAv2~\cite{yang2024da2} & 0.03 & 20.20 \\
DAv3~\cite{lin2025da3} & 0.03 & 20.90 \\
\bottomrule
\end{tabular}
\\[14pt]
...
ViT-B/16 & 27.87 & \textbf{32.76} & \textbf{+4.89} \\
\bottomrule
\end{tabular}
\end{minipage}
\end{tabular}
...
\paragraph{Pseudo-label quality analysis.}

Table~\ref{tab:ablations}(a) evaluates pseudo-label quality directly against ground truth, isolating the pipeline contribution from the training recipe. Depth-guided splitting is necessary: removing depth drops \pqt by 4.5 points. Our best monocular configuration (DAv3, \pqt=20.90) exceeds CUPS Stage-1 thing quality (17.7) using only a single RGB image.

\paragraph{Depth model ablation.}
\label{sec:depth_ablation}

Table~\ref{tab:ablations}(b) confirms that depth quality is the dominant factor in instance pseudo-label quality. Alternative splitting algorithms (Canny, watershed, multiscale Sobel) provide no improvement over standard Sobel given the same depth model.

\paragraph{Training backbone ablation.}

DINOv3 outperforms ResNet-50 by +3.19 PQ (Table~\ref{tab:ablations}(c)). However, at Stage-2, our method exceeds CUPS by only +0.07 PQ (27.87 vs 27.8), suggesting that pseudo-label quality and training recipe---not backbone capacity---are the primary factors before self-training. An ideal control would train CUPS pseudo-labels on the same DINOv3 backbone; however, generating CUPS pseudo-labels requires $\sim$324\,GB of Cityscapes sequence data and 4$\times$ RTX~4090 GPUs, exceeding our infrastructure. We leave this as important future work.

\paragraph{Self-training analysis.}

Self-training gains scale with teacher quality (Table~\ref{tab:ablations}(d)). The \pqt improvement for DINOv3 is +10.9 points (23.2$\to$34.1), while \pqs remains nearly flat (30.6$\to$32.0). This asymmetry arises because self-training primarily bootstraps instance quality beyond what depth-guided pseudo-labels provide.

\paragraph{Qualitative analysis and oracle.}
Figure~\ref{fig:qualitative} shows the qualitative progression from pseudo-labels through Stage-2 and Stage-3 predictions on three Cityscapes validation scenes. Stage-3 produces visibly sharper instance boundaries and recovers semantic classes that are absent from the pseudo-labels.

A per-class PQ breakdown reveals that 7 of the 27 CAUSE classes have PQ below 1\%---effectively zero. If these 7 classes achieved moderate quality (PQ=20\%), overall PQ would rise to 37.94\% (+5.18). These 7 zero-PQ classes account for \textbf{54.7\%} of the total gap to the supervised upper bound (62.3\% PQ). The primary bottleneck is thus a small set of difficult classes---predominantly co-planar thing classes and rare stuff categories---not the well-performing classes (road PQ=92.9\%, sky PQ=85.8\%).

\paragraph{Cross-dataset generalization.}

\begin{table}[t]
\centering
\caption{Cross-dataset evaluation (Stage-3, step~8000, no fine-tuning).}
...
Table~\ref{tab:crossdataset} evaluates generalization without fine-tuning. Mapillary Vistas~v2~\cite{neuhold2017mapillary}---a genuinely different driving dataset with different cameras and global geography---achieves PQ=39.85\% with mIoU=59.8\%, exceeding in-domain Cityscapes---this reflects Mapillary's 124 native classes being mapped to the Cityscapes 19-class schema, which favors large well-represented classes. Transfer quality correlates with domain similarity; COCO (8.1\% PQ) confirms driving-domain features do not generalize to diverse scenes.

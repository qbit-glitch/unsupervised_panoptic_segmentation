...
from util.timm import accuracy, Mixup
...
def train_one_epoch(
    ...
    for data_iter_step, batch_items in enumerate(metric_logger.log_every(data_loader, print_freq, header)):

        samples = batch_items[0]
        targets = batch_items[1]
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            if args.local_rank == 0 and args.wandb is not None:
                try:
                    wandb.log(
                        {"train_loss_step": loss_value_reduce, "train_lr_step": max_lr, "epoch_1000x": epoch_1000x}
                    )
                except ValueError:
                    pass
...
def train_one_epoch_temporal(
    ...
    for data_iter_step, (samples, timestamps, targets) in enumerate(
...
@torch.no_grad()
def evaluate(data_loader, model, device, use_top5=True):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        (acc5,) = accuracy(output, target, topk=(5,)) if use_top5 else (0,)
        # print(acc1, acc5, flush=True)

        batch_size = images.shape[0]
        metric_logger.update(loss=loss.item())
        metric_logger.meters["acc1"].update(acc1.item(), n=batch_size)
        metric_logger.meters["acc5"].update(acc5.item() if use_top5 else 0.0, n=batch_size)
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print(

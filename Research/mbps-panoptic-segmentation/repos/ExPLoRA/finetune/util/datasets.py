...
class SatelliteDataset(Dataset):
    ...
    def build_transform(is_train, input_size, mean, std):
            ...
            t.append(
                (
                    transforms.RandomResizedCrop(
                        input_size, scale=(0.2, 1.0), interpolation=interpol_mode, antialias=antialias_val
                    )
        ...
        t.append(
            transforms.Resize(
                size, interpolation=interpol_mode, antialias=antialias_val
...
class CustomDatasetFromImagesTemporal(SatelliteDataset):
    def __init__(self, csv_path: str):
        ...
        # TODO: CHANGE THIS FOR TEMPORAL DATASET
        self.dataset_root_path = ""

        self.timestamp_arr = np.asarray(self.data_info.iloc[:, 2])
        self.name2index = dict(
            zip([os.path.join(self.dataset_root_path, x) for x in self.image_arr], np.arange(self.data_len))
        )
        ...
        self.scale = transforms.Resize(224, interpolation=transforms.InterpolationMode.BICUBIC, antialias=None)
    ...
    def __getitem__(self, index):
        ...
        for f in temporal_files:
            if f not in self.name2index:
                print(f"Removing: {f}")
                removables.append(f)
        for f in removables:
            temporal_files.remove(f)
        ...
        try:
            if img_as_tensor_1.shape[2] > 224 and img_as_tensor_2.shape[2] > 224 and img_as_tensor_3.shape[2] > 224:
                ...
                img_as_tensor = torch.cat(
                    [img_as_tensor_1[..., :min_w], img_as_tensor_2[..., :min_w], img_as_tensor_3[..., :min_w]], dim=-3
                )
...
class SentinelIndividualImageDataset(SatelliteDataset):
    ...
    def __getitem__(self, idx):
        ...
        single_image_name = selection["image_path"]
        single_image_name = self.path_prefix + single_image_name
        single_image_name = single_image_name.replace("//", "/")

        images = self.open_image(single_image_name)  # (h, w, c)
...
class EuroSat(SatelliteDataset):
    ...
    def __init__(self, file_path, transform, masked_bands=None, dropped_bands=None):
        ...
        self.data_info = pd.read_csv(file_path, header=0)
        # First column contains the image paths
        self.img_paths = np.asarray(self.data_info.iloc[:, 1])
        # Second column is the labels
        self.labels = np.asarray(self.data_info.iloc[:, 0])
    ...
    def open_image(self, img_path):
        ...
        #
        # return img.transpose(1, 2, 0).astype(np.float32)  # (h, w, c)
        img = imread("/scr/samar/" + img_path)  # (h, w, c)
        return img.astype(np.float32)  # (h, w, c)
...
def build_wilds_transform(is_train, input_size, mean, std):
    interpol_mode = transforms.InterpolationMode.BICUBIC
    ...
    if is_train:
        t.append(
            (
                transforms.RandomResizedCrop(input_size, interpolation=interpol_mode, antialias=antialias_val)
                if use_antialias_key
                else transforms.RandomResizedCrop(input_size, interpolation=interpol_mode)
            ),  # 3 is bicubic
        )
        t.append(transforms.RandomHorizontalFlip())
        t.append(transforms.ToTensor())
    ...
    t.append(
        transforms.Resize(size, interpolation=interpol_mode, antialias=antialias_val),
        # to maintain same ratio w.r.t. 224 images
    )
    t.append(transforms.CenterCrop(input_size))
    t.append(transforms.ToTensor())
...
class GlobalWheatLabeledDataWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset):
        self._dataset = dataset
        self.in_c = None

    def __getitem__(self, index: int):
        image, target, *_ = self._dataset[index]
        return image, index
...
def build_fmow_dataset(is_train: bool, args) -> SatelliteDataset:
    ...
    elif args.dataset_type in ("camelyon17", "globalwheat", "iwildcam", "poverty", "rxrx1"):
        dataset = wilds.get_dataset(args.dataset_type, root_dir=csv_path, download=False)
        transform = build_wilds_transform(is_train, args.input_size, IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)
        dataset = dataset.get_subset("train" if is_train else "val", transform=transform)
        if args.dataset_type == "globalwheat":
            dataset = GlobalWheatLabeledDataWrapper(dataset)
        dataset.in_c = 3
    elif args.dataset_type in ("globalwheat_unlabeled", "iwildcam_unlabeled", "camelyon17_unlabeled"):
        dataset_type = args.dataset_type.replace("_unlabeled", "")
        dataset = wilds.get_dataset(dataset_type, root_dir=csv_path, unlabeled=True, download=False)
        transform = build_wilds_transform(is_train, args.input_size, IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)
        dataset = dataset.get_subset("train_unlabeled" if is_train else "val", transform=transform)
        # dataset = dataset.get_subset('extra_unlabeled' if is_train else 'val', transform=transform)
        dataset.in_c = 3
    else:

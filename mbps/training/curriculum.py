...
class TrainingCurriculum:
    ...
    def get_config(self, epoch: int) -> PhaseConfig:
        ...
        if phase == "B":
            # Linear ramp-up of instance weight over Phase B
        ...
        if phase == "C":
            # Gradual warmup of bridge/consistency/PQ weights over first
            # 5 epochs of Phase C to prevent NaN from untrained bridge params.
            warmup_epochs = 5
            epochs_into_c = epoch - self.phase_b_end
            if epochs_into_c <= warmup_epochs:
                ramp = epochs_into_c / warmup_epochs
                return config._replace(
                    gamma=ramp * config.gamma,
                    delta=ramp * config.delta,
                    epsilon=ramp * config.epsilon,
                )

...
class PanopticFPN(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs):
        ...
        Args:
            batched_inputs: a list, batched outputs of :class:`DatasetMapper`.
                ...
                * "depth" (optional): (1, H, W) raw depth map for FiLM conditioning.
                * "pseudo_onehot" (optional): (C, H, W) one-hot pseudo-label targets
        ...
        # M5: Propagate per-pixel confidence weights if present
        pixel_weights = None
        if "confidence_weights" in batched_inputs[0]:
            pw_list = [x["confidence_weights"].to(self.device) for x in batched_inputs]
            pixel_weights = ImageList.from_tensors(
                pw_list,
                self.backbone.size_divisibility,
                0.0,  # pad confidence with 0 (zero weight = ignored)
                self.backbone.padding_constraints,
            ).tensor

        # Approach B: Depth FiLM conditioning (backward-compatible — None when absent)
        depth_tensor = None
        if "depth" in batched_inputs[0]:
            depth_list = [x["depth"].to(self.device) for x in batched_inputs]
            depth_tensor = ImageList.from_tensors(
                depth_list,
                self.backbone.size_divisibility,
                0.0,
                self.backbone.padding_constraints,
        ...
        # Approach B: Stuff-preservation KD pseudo-labels (backward-compatible)
        pseudo_onehot_tensor = None
        if "pseudo_onehot" in batched_inputs[0]:
            po_list = [x["pseudo_onehot"].to(self.device) for x in batched_inputs]
            pseudo_onehot_tensor = ImageList.from_tensors(
                po_list,
                self.backbone.size_divisibility,
                0.0,
                self.backbone.padding_constraints,
    ...
    def inference(
        ...
        # Approach B: Pass depth at inference for DepthFiLMSemSegHead
        depth_tensor = None
        if len(batched_inputs) > 0 and "depth" in batched_inputs[0]:
            depth_list = [x["depth"].to(self.device) for x in batched_inputs]
            depth_tensor = ImageList.from_tensors(
                depth_list,
                self.backbone.size_divisibility,
        ...
        sem_seg_results, sem_seg_losses = self.sem_seg_head(
            features, None, depth=depth_tensor,
        )

    ...
    def forward(self, batch: List[Dict[str, Tensor | Instances]]) -> List[Dict[str, Tensor | Instances]]:
        ...
        for index_batch in range(len(batch_augmented)):
            ...
            if batch_augmented[index_batch]["instances"].gt_masks.tensor.shape[0] != 0:  # type: ignore
                ...
                # Binarize masks after crop (kornia may introduce interpolation artifacts)
                instance_gt = (instance_gt > 0.5).float()
                # Compute valid masks
            ...
            # Save augmented instances — with safety fallback for length mismatches
            n_masks = instance_gt.shape[0]
            if bounding_boxes_gt.shape[0] != n_masks or classes_gt.shape[0] != n_masks:
                # Recompute boxes from cropped binary masks
                bounding_boxes_gt = torch.zeros(n_masks, 4, device=instance_gt.device)
                for i in range(n_masks):
                    ys, xs = torch.where(instance_gt[i] > 0)
                    if len(ys) > 0:
                        bounding_boxes_gt[i] = torch.tensor(
                            [xs.min(), ys.min(), xs.max(), ys.max()],
                            device=instance_gt.device,
                        )
                classes_gt = classes_gt[:n_masks]

"""Cross-dataset assessment: DINOv3 Stage-3 on KITTI Panoptic (OOD evaluation).
...
KITTI Panoptic uses 27 Cityscapes classes (same label format).
"""
import gc
import json
import os
...
from cups.data.cityscapes import (
...
def main():
    ...
    parser.add_argument("--output_json", type=str, default="results/kitti_eval.json",
    ...
    # Validation dataset — KITTI Panoptic uses Cityscapes 27-class format
    validation_dataset = KITTIPanopticValidation(
    ...
    if args.max_images > 0:
        ...
        validation_dataset.labels = validation_dataset.labels[:args.max_images]
    ...
    # Build model with Cityscapes 27-class thing/stuff classes
    ...
    # ── Save results ─────────────────────────────────────────────────────
    class_names = list(CITYSCAPES_CLASSNAMES)
    results = {
        "dataset": "kitti_panoptic",
    ...
    # Print summary
    log.info("\n" + "=" * 60)
    log.info("KITTI Panoptic Cross-Dataset Results (OOD)")
    log.info("=" * 60)
    ...
    for i, name in enumerate(class_names):
        if i < pq_c.shape[0]:
            marker = "T" if i in CITYSCAPES_THING_CLASSES else "S"

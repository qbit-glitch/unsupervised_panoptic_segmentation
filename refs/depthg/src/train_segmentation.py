...
from modules import OriginalLocalHiddenPositiveProjection, LocalHiddenPositiveProjection, FeaturePyramidNet, \
    DinoFeaturizer, DinoFeaturizerWithDepth, DinoFeaturizerUNet, ClusterLookup, ContrastiveCRFLoss, \
    ContrastiveCorrelationLoss, DepthContrastiveCorrelationLoss, norm, sample
...
class LitUnsupervisedSegmenter(pl.LightningModule):
    def __init__(self, n_classes, cfg):
        ...
        if cfg.arch == "feature-pyramid":
            ...
            self.use_depth = True
        elif cfg.arch == "dino_unet":
            # UNet decoder with gravity-aligned geometry skips; geometry loads via use_depth
            # but enters only through the decoder (contrastive loss stays plain STEGO).
            self.net = DinoFeaturizerUNet(dim, cfg)
            self.use_depth = True
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # "dino_unet" routes the 4-ch geometry into the net (as the depth slot) and uses the
        # same 4-tuple forward signature as the depth-aware variant.
        routes_depth = self.arch.__contains__("depth") or self.arch.__contains__("unet")

        input = (img, depth) if self.use_depth and routes_depth else img
        ...
        input_pos = (img_pos, depth_pos) if self.use_depth and routes_depth else img_pos
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            if self.use_depth and self.arch.__contains__("unet"):
                feats, code = self.net((img, batch["depth"]))   # feed val geometry to the decoder
            else:
...
def my_app(cfg: DictConfig) -> None:
    ...
    val_dataset = ContrastiveSegDataset(
        ...
        mask=True,
        return_depth=cfg.use_depth,   # GA-DepthG-UNet: load val geometry for decoder skips at eval
        depth_type=cfg.depth_type,
        cfg=cfg,
    )

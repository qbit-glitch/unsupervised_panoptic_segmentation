...
class CityscapesSeg(Dataset):
    def __init__(self, root, image_set, transform, target_transform, return_depth: bool = False, depth_type="zoedepth"):
        ...
        self.geom_folder_path = join(root, "cityscapes", "geometry", image_set)  # GA-DepthG 4-ch [ĥ,n]
        self.depthpro_folder_path = join(root, "cityscapes", "depth_depthpro", image_set)  # 1-ch DepthPro
    ...
    def __getitem__(self, index):
        ...
        if self.return_depth:
            ...
            if self.depth_type == "zoedepth":
                ...
                depth = to_tensor(Image.open(depth_path))
            elif self.depth_type == "depthpro":  # scalar baseline: 1-ch DepthPro inverse depth
                base = filename.replace("_leftImg8bit", "")
                depth = torch.from_numpy(
                    np.load(join(self.depthpro_folder_path, subfolder, base + ".npy"))).unsqueeze(0)  # (1,H,W)
            elif self.depth_type == "geometry":  # GA-DepthG: 4-ch [ĥ, n]
                base = filename.replace("_leftImg8bit", "")
                depth = torch.from_numpy(
                    np.load(join(self.geom_folder_path, subfolder, base + ".npy")))  # (4,H,W)
            else:
                raise NotImplementedError("Depth type {} not implemented.".format(self.depth_type))

"""Unit tests for evaluation metrics (PyTorch/numpy).
...
class TestPanopticQuality:
    ...
    def test_perfect_prediction(self):
        ...
        assert abs(result.pq - 1.0) < 0.01
        assert abs(result.sq - 1.0) < 0.01
        assert abs(result.rq - 1.0) < 0.01
    ...
    def test_things_vs_stuff(self):
        ...
        assert result.pq_things > 0.0
        assert result.pq_stuff > 0.0
...
class TestSemanticMetrics:
    ...
    def test_compute_miou_perfect(self):
        ...
        from mbps_pytorch.evaluation.semantic_metrics import compute_miou
    ...
    def test_pixel_accuracy(self):
        ...
        assert abs(result.pixel_accuracy - 0.5) < 0.01
...
class TestInstanceMetrics:
    ...
    def test_mask_iou_perfect(self):
        ...
        from mbps_pytorch.evaluation.instance_metrics import compute_mask_iou
    ...
    def test_ap_perfect(self):
        ...
        from mbps_pytorch.evaluation.instance_metrics import compute_ap
    ...
    def test_ap_empty_predictions(self):
        ...
        ap, _, _ = compute_ap(pred_masks, pred_scores, gt_masks, iou_threshold=0.5)
        assert abs(ap - 0.0) < 1e-5

    def test_ap_range(self):
        """AP should be in [0, 1]."""
        from mbps_pytorch.evaluation.instance_metrics import compute_ap_range
        ...
        assert result.ap_mean >= 0.0
        assert result.ap_mean <= 1.0

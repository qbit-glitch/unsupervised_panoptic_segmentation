...
def load_depth_map(
    ...
    """Load a depth .npy map. Returns (512, 1024) or None."""
...
def _resize_grid_feature(arr: np.ndarray, ph: int, pw: int) -> np.ndarray:
    """Resize cached grid features to the requested patch grid if needed."""
    if arr.shape[0] == ph and arr.shape[1] == pw:
        return arr
    arr_t = torch.from_numpy(arr.astype(np.float32)).permute(2, 0, 1).unsqueeze(0)
    arr_t = F.interpolate(arr_t, size=(ph, pw), mode="bilinear", align_corners=False)
    return arr_t.squeeze(0).permute(1, 2, 0).numpy()


def _base_depth_dim(adapter_type: str, adapter_kwargs: dict) -> int:
    """Infer whether an adapter expects raw depth or 16D sinusoidal depth."""
    if adapter_type in ("film", "x", "x2"):
        return int(adapter_kwargs.get("depth_dim", 1))
    depth_dim = int(adapter_kwargs.get("depth_dim", 1))
    return 16 if depth_dim >= 16 else 1


def _load_depth_adapter(ckpt_path: str, device: torch.device) -> torch.nn.Module:
    """Load a DCFA/DCFA-v2 adapter checkpoint for pseudo-label generation."""
    raw = torch.load(ckpt_path, map_location="cpu", weights_only=True)
    if isinstance(raw, dict) and "adapter_type" in raw:
        adapter_type = raw["adapter_type"]
        adapter_kwargs = dict(raw["adapter_kwargs"])
        state_dict = raw["state_dict"]
        if adapter_type == "adaptformer" and "input_mode" not in adapter_kwargs:
            adapter_kwargs["input_mode"] = (
                "project_add" if "depth_proj.weight" in state_dict else "concat"
            )
        adapter = create_adapter(adapter_type, **adapter_kwargs).to(device)
        adapter.load_state_dict(state_dict)
        adapter._v2_type = adapter_type
        adapter._v2_adapter_kwargs = adapter_kwargs
        adapter._train_args = raw.get("train_args", {})
    else:
    ...
    adapter.eval()
    logger.info(
        "Loaded depth adapter: type=%s kwargs=%s",
        getattr(adapter, "_v2_type", "v3"),
        getattr(adapter, "_v2_adapter_kwargs", {}),
    )
    return adapter


def _load_adapter_extras(
    cityscapes_root: str,
    codes_subdir: str,
    split: str,
    city: str,
    ...
    """Load optional adapter inputs from the cached CAUSE-code directory.

    Returns:
        (forward_kwargs, geo_concat), where forward_kwargs are passed directly
        to adapter.forward(), and geo_concat should be concatenated to the depth
        tensor for older concat-style adapters.
    """
    cache_dir = os.path.join(cityscapes_root, codes_subdir, split, city)
    fwd_kwargs = {}
    geo_concat = []

    def load_grid(name: str, channels: int) -> torch.Tensor:
        path = os.path.join(cache_dir, f"{stem}_{name}.npy")
        if os.path.isfile(path):
            arr = np.load(path).astype(np.float32)
            arr = _resize_grid_feature(arr, ph, pw)
        else:
            logger.warning("Missing cached %s for %s/%s; using zeros.", name, city, stem)
            arr = np.zeros((ph, pw, channels), dtype=np.float32)
        return torch.from_numpy(arr.reshape(1, ph * pw, channels)).to(device)

    def load_gradients() -> torch.Tensor:
        depth_path = os.path.join(cache_dir, f"{stem}_depth.npy")
        if os.path.isfile(depth_path):
            depth = np.load(depth_path).astype(np.float32)
            if depth.shape != (ph, pw):
                depth = np.array(Image.fromarray(depth).resize((pw, ph), Image.BILINEAR))
            grads = compute_sobel_gradients(depth).astype(np.float32)
        else:
            logger.warning("Missing cached depth for gradients %s/%s; using zeros.", city, stem)
            grads = np.zeros((ph * pw, 3), dtype=np.float32)
        return torch.from_numpy(grads.reshape(1, ph * pw, 3)).to(device)

    if adapter_type in ("cross_attn", "x", "x2"):
        fwd_kwargs["dino768"] = load_grid("dino768", 768)

    if adapter_type in ("x", "x2"):
        geo_dim = int(adapter_kwargs.get("geo_dim", 0))
        if geo_dim >= 3:
            fwd_kwargs["normals"] = load_grid("normals", 3)
        if adapter_type == "x2" and geo_dim >= 6:
            fwd_kwargs["gradients"] = load_gradients()
        fwd_kwargs["spatial_shape"] = (ph, pw)
    elif adapter_type == "window_attn":
        fwd_kwargs["spatial_shape"] = (ph, pw)

    concat_extra_dim = 0
    if adapter_type in ("v3", "deep", "adaptformer", "window_attn", "cross_attn"):
        depth_dim = int(adapter_kwargs.get("depth_dim", 1))
        base_dim = 16 if depth_dim >= 16 else 1
        concat_extra_dim = max(0, depth_dim - base_dim)
    elif adapter_type == "film":
        concat_extra_dim = int(adapter_kwargs.get("geo_dim", 0))

    if concat_extra_dim >= 3:
        geo_concat.append(load_grid("normals", 3))
    if concat_extra_dim >= 6:
        geo_concat.append(load_gradients())

    return fwd_kwargs, geo_concat


def _adapter_depth_tensor(
    depth_flat: np.ndarray,
    adapter_type: str,
    adapter_kwargs: dict,
    device: torch.device,
) -> torch.Tensor:
    depth_t = torch.from_numpy(depth_flat.astype(np.float32)).unsqueeze(0).to(device)
    if _base_depth_dim(adapter_type, adapter_kwargs) >= 16:
        return sinusoidal_depth_encode(depth_t)
    return depth_t.unsqueeze(-1)


def _apply_depth_adapter(
    ...
    """Apply a loaded DCFA adapter to patch-grid 90D CAUSE codes."""
    adapter_type = getattr(adapter, "_v2_type", "v3")
    adapter_kwargs = getattr(adapter, "_v2_adapter_kwargs", {"depth_dim": 1})
    codes_t = torch.from_numpy(feats_90.astype(np.float32)).unsqueeze(0).to(device)
    depth_t = _adapter_depth_tensor(depth_ds.reshape(-1), adapter_type, adapter_kwargs, device)
    fwd_kwargs, geo_concat = _load_adapter_extras(
        cityscapes_root, codes_subdir, split, city, stem, ph, pw,
        adapter_type, adapter_kwargs, device,
    )
    if geo_concat:
        depth_t = torch.cat([depth_t] + geo_concat, dim=-1)
    with torch.no_grad():
        adjusted = adapter(codes_t, depth_t, **fwd_kwargs)
...
def fit_kmeans_with_depth(
    ...
    for entry in tqdm(images, desc="Extracting features"):
        ...
        depth_map = None
        if variant != "none" or adapter is not None:
        ...
        if adapter is not None:
            ...
            feats_90 = _apply_depth_adapter(
                adapter, feats_90, depth_ds,
                cityscapes_root, codes_subdir, "val",
                entry["city"], entry["stem"], ph, pw, device,
            )
    ...
    for entry in tqdm(images, desc=f"Generating {split}"):
        ...
        if adapter is not None:
            ...
            adjusted = _apply_depth_adapter(
                adapter, feats_90, depth_ds,
                cityscapes_root, codes_subdir, split,
                entry["city"], entry["stem"], ph, pw, device,
            )
            adjusted_patch = torch.from_numpy(
                adjusted.T.reshape(90, ph, pw).astype(np.float32)
            ).unsqueeze(0).to(device)
            feat_map = F.interpolate(
                adjusted_patch, size=(new_h, new_w),
...
def main() -> None:
    ...
    if args.load_centroids:
        ...
        np.savez(centroids_path, centroids=centroids_norm,
                 ...
                 adapter_checkpoint=args.adapter_checkpoint or "",
    ...
    generate_pseudolabels_with_depth(
        ...
        adapter=adapter, codes_subdir=args.codes_subdir,
    )

...
def select_foreground_proposals(
    ...
    """Given a list of N Instances (for N images), each containing a `gt_classes` field, return a list of Instances that
...
class ROIHeads(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            num_classes (int): number of foreground num_clusters_things (i.e. background is not included)
    ...
    def _sample_proposals(
        ...
        """Based on the matching between N proposals and M groundtruth, sample the proposals and set their
    ...
    def label_and_sample_proposals(self, proposals: List[Instances], targets: List[Instances]) -> List[Instances]:
        ...
        from proposals and groundtruth boxes, with a fraction of positives that is no larger than
...
class Res5ROIHeads(ROIHeads):
    ...
    def from_config(cls, cfg, input_shape):
        ...
        # Compatibility with old moco code. Might be useful.
    ...
    def forward(
        ...
        if self.training:
            del features
            losses = self.box_predictor.losses(predictions, proposals)  # type: ignore
            if self.mask_on:
                ...
                del box_features
                losses.update(self.mask_head(mask_features, proposals))  # type: ignore
            return [], losses
        else:
    ...
    def forward_with_given_boxes(
        ...
        if self.mask_on:
            ...
            return self.mask_head(x, instances)  # type: ignore
        else:
            return instances
...
class CustomStandardROIHeads(ROIHeads):
    """It's "standard" in a sense that there is no ROI transform sharing or feature sharing between tasks. Each head
    independently processes the input features by each head's own pooler and head.

    This class is used by most models, such as FPN and C5. To implement more models, you can subclass it and implement a
    different :meth:`forward()` or a head.
    """
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances]):
        """Forward logic of the box prediction branch. If `self.train_on_pred_boxes is True`, the function puts
        predicted boxes in the `proposal_boxes` field of `proposals` argument.
        ...
        if self.training:
            if self.use_droploss and not no_gt_found:
                ...
                losses = self.box_predictor.losses(predictions, proposals, weights=weights.detach())  # type: ignore
            else:
                losses = self.box_predictor.losses(predictions, proposals)  # type: ignore
            if self.train_on_pred_boxes:  # default is false
                with torch.no_grad():
                    pred_boxes = self.box_predictor.predict_boxes_for_gt_classes(predictions, proposals)  # type: ignore
    ...
    def _forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances]):
        ...
        if self.mask_pooler is not None:
            features = [features[f] for f in self.mask_in_features]  # type: ignore
    ...
    def _forward_keypoint(self, features: Dict[str, torch.Tensor], instances: List[Instances]):
        ...
        if self.keypoint_pooler is not None:
            features = [features[f] for f in self.keypoint_in_features]  # type: ignore
            boxes = [x.proposal_boxes if self.training else x.pred_boxes for x in instances]
            features = self.keypoint_pooler(features, boxes)
        else:
            features = {f: features[f] for f in self.keypoint_in_features}  # type: ignore
        return self.keypoint_head(features, instances)  # type: ignore

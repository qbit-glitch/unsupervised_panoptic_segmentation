...
Usage:
    python scripts/run_ablations.py --config configs/cityscapes_gcs.yaml
    python scripts/run_ablations.py --config configs/cityscapes_gcs.yaml --ablations no_mamba no_bicms
    python scripts/run_ablations.py --config configs/cityscapes_gcs.yaml --seeds 42 123 456
...
def merge_configs(base_path: str, ablation_path: str) -> Dict[str, Any]:
    ...
    Args:
        base_path: Path to base dataset config (e.g. cityscapes_gcs.yaml).
...
def run_ablation(
    ...
    logging.info(f"[{ablation_name}/seed={seed}] Command: {' '.join(train_cmd)}")
    ...
    if dry_run:
        ...
        logging.info(f"  [DRY RUN] Would execute above command")
        return result
    ...
    try:
        ...
        result["status"] = "error"
        result["error"] = str(e)
        logging.error(f"  [{ablation_name}/seed={seed}] ERROR: {e}")
...
def main():
    ...
    for abl_name, abl_desc in selected:
        ...
        if not os.path.exists(abl_config):
            logging.warning(f"Config not found: {abl_config}, skipping")
            continue

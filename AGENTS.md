...
- Do not rebuild graphify automatically after code edits. Graphify rebuilds are expensive in this repo.
- Rebuild graphify only when the user explicitly asks for it, or when graph freshness is essential to the current task and the user has approved the time cost.

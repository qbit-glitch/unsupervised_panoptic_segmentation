...
def get_correlation_depth(lookup):
    depth = 0
    if lookup["center"]["method"] == "block":
        depth += ((2 * lookup["center"]["size"] + 1)**2) * lookup["cen_nei_pyramid"]
    elif lookup["center"]["method"] == "circular":
        depth += len(lookup["center"]["radii"]) * lookup["center"]["size"] * lookup["cen_nei_pyramid"] +  lookup["cen_nei_pyramid"]
    
    if lookup["neighbor"]["method"] == "block":
        depth += 8 * ((2 * lookup["neighbor"]["size"] + 1)**2) * lookup["cen_nei_pyramid"]
    elif lookup["neighbor"]["method"] == "gaus":
        depth += 8 * lookup["neighbor"]["size"] * lookup["cen_nei_pyramid"]
    
    if lookup["random"]["method"] != "None":
        if lookup["random"]["pyramid"]:
            depth += lookup["random"]["size"] * lookup["cen_nei_pyramid"]
        else:
            depth += lookup["random"]["size"]
    print(depth)

...
class StatsLogger:
    def __init__(self, name, current_steps, phase):
        ...
        self.logger = logging.getLogger("raft.stats") # this is a logger of type "raft". It can be more strict.
        ...
        with open(txtfile_path, "w") as file:
            ...
            if checkpoint_config["newer"] == None:
                ...
                if os.path.exists(file_path_to_be_removed):
                    ...
                    logger = logging.getLogger("raft.saving")
...
def fetch_model(config, phase):
    #comment *nn.DataParallel* for determinism
    model = nn.DataParallel(RAFT(config), device_ids=config["gpus"])

    print("Parameter Count: %d" % count_parameters(model))           

    model.cuda() #comment for determinism
    model.train()
...
def training_step(config, model, data_group, optimizer, phase, scaler, loss_module, cur_step, ar_augmentor, flow_loss_current_weight):
    ...
    for data in data_group:
        ...
        if config["occ_method"][phase] == "brox":
            ...
            occlusions = get_occu_mask_backward(flow_predictions,flow_bw_predictions, mask_out=config["mask_out"], detach=config["detach"])
            occlusions_bw = get_occu_mask_backward(flow_bw_predictions,flow_predictions, mask_out=config["mask_out"], detach=config["detach"])  

        if config["train"]["loss"]["ar"]:
            with torch.no_grad():
                flow_predictions_teacher, flow_bw_predictions_teacher = model(image1=image1, image2=image2, iters=iterations, bw=config["bw"])
        ...
        scaler.scale(metrics["total_loss"]).backward() #scale based on the gradient accumulation step.
...
def train_single_phase(model, data_generator, optimizer, scheduler,loss_module, init_local_step, num_steps, stats_logger, training_step_fn, save_fn, phase):
    ...
    if config["train"]["loss"]["ar"]:
        ...
        activation_start_iter = (config["train"]["loss"]["ar_start"][phase]/100.)*num_steps ##move out of each iter
        last_increasing_iter = activation_start_iter + (config["train"]["loss"]["ar_increasing"]/100.)*num_steps ## move out of each iter
    ...
    for local_step in range(init_local_step, num_steps - 1):
        ...
        metrics = training_step_fn(model, data_group, optimizer, phase, loss_module=loss_module, cur_step=local_step, ar_augmentor=ar_augmentor, flow_loss_current_weight=flow_loss_current_weight)
        ...
        if (local_step + 1) % VAL_FREQ == VAL_FREQ - 1: #save checkpoint now
            ...
            for index, ev in enumerate(evals):
                if ev == 'chairs':
                    results.update(evaluate.validate_chairs(model.module, iters = eval_iters_list[index]))
                elif ev == 'sintel':
                    results.update(evaluate.validate_sintel_during_training(model.module, iters = eval_iters_list[index], warm=True))
                elif ev == 'kitti':
                    results.update(evaluate.validate_kitti(model.module, iters = eval_iters_list[index]))
                else:
...
def train_phases(init_phase, num_phases, local_step_of_init_phase, stats_logger, state_dict, datasets, num_steps, model_fn, data_fn, optimizer_scheduler_fn, training_step_fn, save_fn):
    ...
    for phase in range(init_phase, num_phases):
        loss_module = nn.DataParallel(FlowLoss(config["train"]["loss"], phase=phase), device_ids=config["gpus"])
        ...
        train_single_phase(model, data, optimizer, scheduler, loss_module, local_step, num_steps[phase], stats_logger, training_step_fn, lambda model, step, policy: save_fn(model, step, phase, policy), phase)
...
def train(config):
    ...
    lambda model, data, optimizer, phase, loss_module, cur_step, ar_augmentor, flow_loss_current_weight: training_step(config, model, data, optimizer, phase, scaler, loss_module, cur_step, ar_augmentor, flow_loss_current_weight),

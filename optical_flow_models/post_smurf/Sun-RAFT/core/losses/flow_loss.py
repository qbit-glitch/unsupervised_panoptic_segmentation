...
class FlowLoss(nn.modules.Module):
    ...
    def forward(self, flow12_list, flow21_list, occ12_list, occ21_list, img1, img2, flow_predictions_of_aug_imgs, flow_gt, teacher_student_masking=False, teacher_mask=None, flow_loss_current_weight=0):
        ...
        return (loss*batch_size).unsqueeze(dim=0), (L_ph*batch_size).unsqueeze(dim=0), (L_sm*batch_size).unsqueeze(dim=0), (flow_loss*batch_size).unsqueeze(dim=0), (epe*batch_size).unsqueeze(dim=0) # to handle proper mean over all GPUS using DataParallel!

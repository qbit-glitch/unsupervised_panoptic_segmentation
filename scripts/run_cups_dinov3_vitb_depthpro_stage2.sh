#!/bin/bash
# CUPS Stage-2: DINOv3 ViT-B/16 — DepthPro pseudo-labels
# Backbone: DINOv3 ViT-B/16 (same as successful k80 run: Stage-2 PQ=27.865%)
# Pseudo-labels: cups_pseudo_labels_depthpro/ (DepthPro τ=0.01 + k80 semantics)
#   Pseudo-label PQ: 28.40, PQ_things: 23.35 (+2.45 over DA3, +3.94 over SPIdepth)
# Target: > 29% PQ at Stage-2 (vs 27.865% with k80+SPIdepth)
# eff_bs: 2 GPUs × bs=1 × accum=8 = 16 (same as k80 ViT-B run)
#
# ─── Deploy (run locally) ─────────────────────────────────────────────────────
#   scp refs/cups/configs/train_cityscapes_dinov3_vitb_depthpro_8k_2gpu.yaml \
#       santosh@172.17.254.146:/media/santosh/Kuldeep/panoptic_segmentation/refs/cups/configs/
#   scp scripts/run_cups_dinov3_vitb_depthpro_stage2.sh \
#       santosh@172.17.254.146:/media/santosh/Kuldeep/panoptic_segmentation/scripts/
#
# ─── Launch (on remote) ───────────────────────────────────────────────────────
#   nohup bash /media/santosh/Kuldeep/panoptic_segmentation/scripts/run_cups_dinov3_vitb_depthpro_stage2.sh \
#     > /dev/null 2>&1 &
#   echo "PID: $!"
#
# ─── Monitor ──────────────────────────────────────────────────────────────────
#   tail -f /media/santosh/Kuldeep/panoptic_segmentation/experiments/cups_dinov3_vitb_depthpro_8k_stage2.log
#
# ─── Resume (if interrupted) ──────────────────────────────────────────────────
#   bash run_cups_dinov3_vitb_depthpro_stage2.sh "/path/to/.../checkpoints/last.ckpt"
#
# ─── After Stage-2 completes ──────────────────────────────────────────────────
#   1. Find best checkpoint:
#      ls /media/santosh/Kuldeep/panoptic_segmentation/experiments/experiments/
#         cups_dinov3_vitb_depthpro_8k_stage2/*/checkpoints/best_pq_step=*.ckpt
#   2. Launch Stage-3 with that checkpoint
...
CONFIG="${WORK_DIR}/refs/cups/configs/train_cityscapes_dinov3_vitb_depthpro_8k_2gpu.yaml"
LOG_FILE="${WORK_DIR}/experiments/cups_dinov3_vitb_depthpro_8k_stage2.log"
PSEUDO_DIR="${WORK_DIR}/datasets/cityscapes/cups_pseudo_labels_depthpro"

cd "${WORK_DIR}/refs/cups"
mkdir -p "${WORK_DIR}/experiments"

echo "=== CUPS Stage-2: DINOv3 ViT-B/16 + DepthPro pseudo-labels ===" | tee "${LOG_FILE}"
...
echo "Launching Stage-2 (8000 steps, DINOv3 ViT-B/16, DepthPro labels)..." | tee -a "${LOG_FILE}"
...
echo "=== Stage-2 DINOv3 ViT-B + DepthPro complete: $(date) ===" | tee -a "${LOG_FILE}"

# Print best checkpoint path for Stage-3
echo "" | tee -a "${LOG_FILE}"
echo "Best checkpoint(s):" | tee -a "${LOG_FILE}"
find "${WORK_DIR}/experiments" -path "*/cups_dinov3_vitb_depthpro_8k_stage2/*/checkpoints/best_pq_step=*.ckpt" \
